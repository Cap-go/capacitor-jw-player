...
const pluginName = 'CapgoCapacitorJwPlayer';
const frameworkNames = ['JWPlayerKit', 'GoogleCast'];
const phaseName = 'Capgo dSYMs for JWPlayerKit and GoogleCast';
...
function shellScript() {
  return [
    ...
    '    if ! cp -R "$dsym_path" "$archive_dsyms/"; then',
    '      copy_failed=1',
    '    fi',
    '  done',
    '  return "$copy_failed"',
    ...
    '    if ! copy_to_archive "$dsym_path" "$framework"; then',
    '      log "Failed to copy $framework.framework.dSYM into the archive."',
    '      missing_dsyms=1',
    '    fi',
    '    continue',
    '  fi',
